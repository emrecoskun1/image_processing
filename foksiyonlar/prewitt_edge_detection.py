...
def prewitt_edge_detection(image_path):
    ...
    edge_intensity = [[(gradient_x[y][x]**2 + gradient_y[y][x]**2)**0.5 for x in range(width)] for y in range(height)]
    
    # Kenar yoğunluğunu 0 ile 255 arasında normalize et
    max_intensity = max(max(row) for row in edge_intensity)
    edge_intensity = [[int(pixel * 255 / max_intensity) for pixel in row] for row in edge_intensity]
...
def convolution(image, kernel):
    ...
    # Kenar pikselleri için sıfırları koru
    pad_height, pad_width = k_height // 2, k_width // 2
    padded_image = [[0]*(width + 2 * pad_width) for _ in range(pad_height)] + image + [[0]*(width + 2 * pad_width) for _ in range(pad_height)]
    ...
    for y in range(height):
        for x in range(width):
            output[y][x] = sum(padded_image[y + i][x + j] * kernel[i][j] for i in range(k_height) for j in range(k_width))
